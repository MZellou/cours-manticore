"""Test Phase 3 SQL (pgRouting) — parsed dynamically from corriges/phase_3.qmd."""
...
QUERIES = extract_queries("corriges/phase_3.qmd", lang_filter=["sql"])


@pytest.mark.needs_gold_dumps
@pytest.mark.parametrize("query", QUERIES, ids=lambda q: q.id)
def test_sql(pg_conn, mission_pois_ready, has_gold_dumps, routing_vertices, sample_edge_id, query):
    if not has_gold_dumps:
        pytest.skip("No gold dumps")

    code = query.code
    if is_template(code):
        code = resolve_templates(code, **routing_vertices, **sample_edge_id)
        if is_template(code):
