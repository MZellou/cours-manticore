...
:::{.callout-important title="Page de référence"}
Cette page liste les statistiques détaillées des 13 EPCIs. **Ne la lisez pas linéairement** — survolez-la, et **lisez en détail uniquement la section de votre EPCI assigné**. Revenez ici en Phase 3 pour des références juste-à-temps.
:::
...
| **pylone** | 265 143 | Point | `hauteur` | Logistique : réseau électrique (265k pylônes !) |
| **poste_de_transformation** | 4 104 | Point | `importance` (1-6) | Logistique : postes critiques (importance ≤ 3) |
| **ligne_electrique** | 14 470 | Line | `voltage` | Logistique : lignes THT (400kV, 225kV) |
...
| **reservoir** | 91 737 | Surf. | `nature`, `volume`, `hauteur` | Logistique : réservoirs industriels, châteaux d'eau |
| **terrain_de_sport** | 139 272 | Surf. | `nature`, `nature_detaillee` | Peu pertinent militairement |
| **canalisation** | 7 906 | Line | `nature` | Logistique : hydrocarbures (1 169), autres (6 737) |
...
| **troncon_de_voie_ferree** | 104 717 | Line | `nature`, `usage`, `electrifie`, `nombre_de_voies`, `vitesse_maximale` | Logistique : voies fret, LGV |
...
| Logistique | Industriel et commercial + (via tables dédiées) | Zone industrielle, Usine, Marché + postes HT, lignes THT, etc. |
...
### 3.2 Centrales électriques

Les centrales électriques (dont nucléaires) sont dans BDTOPO via `zone_d_activite_ou_d_interet` WHERE `nature = 'Centrale électrique'`. Selon l'EPCI, certaines peuvent être proches mais hors périmètre — utilisez `ST_DWithin` avec un rayon large pour les capter.
...
![POI counts](../../assets/img/theory/chart_poi_counts.png)
...
![Road network](../../assets/img/theory/chart_road_network.png)

**Deux couleurs** : bleu = tronçons en zone urbaine, orange = rural. Les EPCIs métropoles (Lyon, Lille, Bordeaux) ont un réseau quasi-intégralement urbain. Le Cotentin et le Pays Basque sont profondément ruraux. Utile pour le TD Logistique : les livraisons en zone rurale dépendent des chemins (plus lents, plus vulnérables).

#### Chaleur : alignment des rôles

![Role heatmap](../../assets/img/theory/chart_role_heatmap.png)

**Lignes** = rôles (Attaque, Défense, Logistique). **Couleurs** = pertinence des tables pour ce rôle (échelle log). Un coefficient élevé (rouge) signifie que cette table contient beaucoup de POIs utiles à ce rôle dans la majorité des EPCIs. Par exemple : Logistique → `reservoir` (eau potable, industriel) est pertinent partout.

#### Composition du réseau routier par type de voie

![Road composition](../../assets/img/theory/chart_road_composition.png)
...
![Data inventory](../../assets/img/theory/chart_data_inventory.png)
...
#### Densité du réseau routier (tronçons / km²)

![Road density](../../assets/img/theory/chart_road_density.png)

**Normalisation par la surface** de l'EPCI : c'est elle qui révèle la vraie densité urbaine. Lille et Lyon écrasent les comptages bruts, mais une fois ramené au km² on voit que Brest, Le Havre et Dunkerque sont en réalité parmi les territoires les plus densément maillés. Les EPCIs étendus (Cotentin, Pays Basque) chutent fortement : leur grand volume de routes s'étale sur de grandes surfaces.

#### Choke-points candidats (ponts, tunnels, barrages)

![Choke points](../../assets/img/theory/chart_choke_points.png)

**Ouvrages d'art = points uniques de défaillance.** Un pont coupé isole, un tunnel obstrué redirige tout le trafic. Cette barre cumulée prépare la Phase 3 : repérez les EPCIs riches en ouvrages (Lyon, Rouen, Strasbourg = nombreux ponts urbains) et ceux où chaque ouvrage est critique (Cotentin, Brest = peu d'ouvrages mais réseau péninsulaire). Utile pour scénariser des destructions Phase 3.

#### Pyramide d'importance routière

![Road importance](../../assets/img/theory/chart_road_importance.png)

**Importance BDTOPO** = hiérarchie d'usage : 1-2 = national/régional, 3 = départemental, 4-6 = local/desserte. La majorité écrasante des EPCIs est composée de routes locales (gris). Les % de routes "stratégiques" (rouge/orange) sont faibles partout mais varient : Lille, Strasbourg, Bordeaux ont une bonne épine dorsale ; Pays Basque et Cotentin reposent surtout sur du desserte local. Complète la composition par *nature* : ici on lit la **fonction**, pas la qualité de chaussée.

#### Réseau électrique : pylônes, postes HT, lignes THT

![Electrical resilience](../../assets/img/theory/chart_electrical_resilience.png)

**Trois indicateurs normalisés** (1 = max sur les 13 EPCIs) : pylônes (densité de support), postes de transformation (nœuds critiques basse-tension), lignes 225/400 kV (transport haute tension). Lyon et Strasbourg dominent les postes ; Lyon dépasse en lignes THT. Pour le rôle Logistique (volet énergie), ces ratios indiquent où le réseau est dense (et donc vulnérable à un point unique) versus distribué.
...
| `equipement_de_transport` | `nature='Tour de contrôle aérien'` | Tours de contrôle | ~0-1 |
...
| `construction_ponctuelle` | `nature='Phare' OR (nature='Autre construction élevée' AND nature_detaillee='Tour de guet')` | Points d'observation |
| `construction_surfacique` | `nature='Pont'` | Ponts stratégiques |

### Rôle LOGISTIQUE

**Objectif** : Optimiser les flux logistiques et sécuriser le réseau énergétique.
...
| `reservoir` | `nature='Réservoir industriel'` | Réservoirs industriels |
...
| `construction_ponctuelle` | `nature='Puits d''hydrocarbures'` | Puits de pétrole, de gaz |
...
- **Le Havre** : 1 737 réservoirs (port pétrolier !) → parfait rôle Logistique.
- **Dunkerque** : 901 réservoirs + 1 151 voies ferrées → parfait rôle Industries/Logistique.
...
- **Île de Ré** : 0 gares, 0 aérodromes → rôle Défense ou Logistique maritime uniquement.
- **Ajaccien** : 0 autoroute → pas de réseau à grande vitesse.
- **Grenoble** : cuvette, 1 713 restrictions de poids → zone montagneuse.

### 5.3 Centrales nucléaires (non dans BDTOPO)
Les centrales ne sont pas des POI BDTOPO. Pour le rôle Logistique (volet énergétique), on peut :
