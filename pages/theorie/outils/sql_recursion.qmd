...
#### Profondeur réelle de l'ontologie BDTOPO

![Ontologie BDTOPO](../../assets/img/theory/chart_ontology.png)

**Hiérarchie à 3 niveaux** : `Database` (ex. *Transport*, *Hydrographie*) → `Object` (ex. *Tronçon de route*) → `Detail` (ex. *Bretelle*, *Type autoroutier*). Le graphique montre, pour chaque Database, le nombre d'Objects (orange) et de Details feuilles (bleu) qui en dépendent. C'est exactement la structure parcourue par la CTE récursive : plus le tronc est ramifié, plus la récursion fait d'itérations avant d'atteindre le point fixe.
