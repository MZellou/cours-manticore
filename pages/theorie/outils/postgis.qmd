...
## Clustering spatial — ST_ClusterDBSCAN

DBSCAN (Density-Based Spatial Clustering) regroupe les points denses **sans avoir à fixer le nombre de clusters**. Les points trop isolés sont des outliers (cluster `-1`). Deux paramètres seulement :
...
FROM mission_pois
WHERE role = 'attaque';
-- cluster_id = -1  → point isolé (outlier, pas assez de voisins dans eps)
-- cluster_id >= 0  → appartient au cluster numéroté
```

::: {.callout-note}
`ST_ClusterDBSCAN` est une **fonction de fenêtrage** (`OVER ()`). Elle ne filtre pas les outliers — elle leur assigne `-1`. Pour ne garder que les clusters réels : `WHERE cluster_id >= 0`.
:::
