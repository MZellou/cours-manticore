...
→ [Corrigé](../corriges/phase_1.qmd#t0a--explorer-les-donnes-sources-avec-duckdb)
...
**Protocole** : chaque rôle exporte ses POIs en CSV (`\copy`), partage le fichier (clé USB, scp), puis un étudiant importe les 4 CSV dans sa base.

*Indice : `\copy (SELECT * FROM mission_pois WHERE role='votre_role') TO 'pois.csv' CSV HEADER`*

**Vérifiez** : `SELECT role, source, count(*) FROM mission_pois GROUP BY role, source` → 4 rôles présents.
