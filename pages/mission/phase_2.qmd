...
**Pré-requis Phase 1** : la table `mission_pois` est non-vide pour les 4 rôles ([Checkpoint Phase 1](checkpoint_1.qmd)).
:::
...
Le script fait `MERGE (p:POI {cleabs: $cleabs})`. Or **certains `cleabs` sont partagés** entre rôles : un réservoir industriel apparaît à la fois dans `mission_pois` pour `ravitaillement` (capacité de stockage) **et** pour `energie` (cuve de fioul). Une gendarmerie idem entre `defense` et `attaque`. `MERGE` matche le nœud existant et **écrase la propriété `role`** au lieu de créer un doublon — donc le dernier rôle migré "gagne".
...
1. **Clé composite** : `MERGE (p:POI {cleabs: $cleabs, role: $role})` → 2 nœuds distincts pour le réservoir, un par rôle. Simple, mais duplique la géométrie.
2. **Multi-label** : un seul nœud, on `SET p:Ravitaillement, p:Energie` ou on stocke `roles: [...]` en liste. Plus propre, mais les requêtes `MATCH (p:POI {role: 'energie'})` doivent devenir `WHERE 'energie' IN p.roles`.
