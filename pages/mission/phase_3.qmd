...
**Mode** : Groupe (les 3 rôles collaborent sur la simulation).
**Durée estimée** : 5h (benchmark 1h30 + simulation itérative 2h + analyse réseau 1h + T-Synthèse 30min)

::: {.callout-tip title="La simulation impacte les 3 rôles différemment"}
Chaque agent mesure **son** indicateur lors de la simulation d'attaque :

- **Attaque** : combien de cibles deviennent déconnectées du reste du réseau ?
- **Défense** : combien d'hôpitaux / ponts sont isolés après les coupes ?
- **Logistique** : combien de corridors énergétiques / d'approvisionnement sont rompus ?

Comparez vos indicateurs en fin de phase : le même graphe coupé n'a pas le même impact selon le rôle.
:::

::: {.callout-note title="📚 Théorie pré-requise"}
- [APOC — avancé](../theorie/avance/apoc.qmd) — `betweenness`, `expandConfig`, `PROFILE` (essentiel)
- [APOC — essentiels](../theorie/outils/apoc_basics.qmd) — `dijkstra`, `subgraphAll` (rappel)
- [SQL — limites & EXPLAIN](../theorie/outils/sql_recursion.qmd) — lire un plan `EXPLAIN ANALYZE` / `PROFILE`
- [pgRouting et r2gg](../theorie/outils/pgrouting.qmd) — `pgr_connectedComponents`, simulation de coupes

**Pré-requis Phase 2** : graphe ways + ways_vertices_pgr OK, Neo4j peuplé ([Checkpoint Phase 2](checkpoint_2.qmd)).
:::

[← Transition Phase 2 → Phase 3](transition_2_3.qmd) — *à lire d'abord pour cadrer la démarche.*
...
## T0 — État initial du graphe (15 min) — *requis avant simulation* {#sec-t0-etat-initial}

::: {.callout-warning title="🩺 Avant de simuler, mesurer"}
Avant de **couper** des arêtes, il faut connaître **l'état de santé** du graphe. La simulation de destruction (Partie B) consiste à mesurer un **delta** : sans baseline, le delta n'a pas de sens.

Trois métriques à relever **maintenant**, sur le graphe intact :
:::

**1. Combien d'arêtes sont déjà impassables voiture ?**
...
> Sur Brest : ~28 % des arêtes ont déjà `cost = -1` (pistes, sentiers, voies privées, tarmac aéroport — convention r2gg). Vous ne partez **pas** d'un graphe parfait.

**2. Combien de composantes connexes dans le graphe carrossable ?**
...
> Si > 1 : le réseau routier est **déjà fragmenté**. Certaines parties de l'EPCI ne se rejoignent pas en voiture (îles, péninsules, zones militaires fermées…).

**3. Combien de POIs sont sur des sommets isolés (récap T2.5) ?**

Reprenez la requête de [Phase 2 §T2.5](../mission/phase_2.qmd#sec-reality-check) — les POIs « inatteignables voiture » sont autant de **paires Dijkstra qui retourneront NULL** dans la suite.

::: {.callout-tip title="🧠 Pédagogie : le delta est ce qui compte"}
Quand vous couperez 3 arêtes en T6, vous augmenterez le `pct_impassables` de ~28 % à ~28.01 %. Ce qui compte, c'est :

- de **combien** la distance moyenne entre POIs augmente
- combien de **nouvelles** composantes connexes apparaissent
- combien de **nouveaux** POIs deviennent isolés

Un graphe déjà fragmenté est **plus fragile** : couper une arête sur un pont reliant deux composantes peut isoler 30 % de l'EPCI d'un coup. C'est exactement ce que recherche un agent **Attaque** — et ce que doit anticiper un agent **Défense**.
:::
...
→ [Corrigé](../corriges/phase_3.qmd#t1--choke-points)
...
Trouvez le chemin attaque → logistique → défense le plus court.
...
→ [Corrigé T2a-T2c](../corriges/phase_3.qmd#t2a--tous-les-chemins-entre-deux-roles)
...
→ [Corrigé](../corriges/phase_3.qmd#t3--requetes-benchmark)

::: {.callout-warning title="⚖️ Sémantique du COUNT : SQL et Cypher ne comptent pas la même chose"}
Sur la requête 1, vous obtiendrez par exemple **SQL = 11** et **Cypher = 10**. Pas un bug — une différence sémantique :

```sql
WITH RECURSIVE hierarchy AS (
  SELECT id FROM bdtopo_ontology WHERE name = 'Tronçon de route'   -- ← inclut la racine
  UNION ALL
  SELECT child.id FROM bdtopo_ontology child JOIN hierarchy h ON child.parent_id = h.id
)
SELECT count(*) FROM hierarchy;   -- 11 = 1 racine + 10 enfants
```

```cypher
MATCH (n)-[:EST_SOUS_TYPE_DE*1..3]->(:ClasseOntologie {name: 'Tronçon de route', obj_type: 'Database'})
RETURN count(n);   -- 10 = juste les descendants, la racine n'est pas un "n"
```

Le `WITH RECURSIVE` SQL **inclut** la seed row par défaut (le `WHERE name = ...` la matche). Le pattern Cypher `*1..3` exige un saut au minimum (`1..`), donc la source du `MATCH` n'est jamais elle-même.

**Comment benchmarker honnêtement** ?

- Soit on aligne : `MATCH (n)-[:EST_SOUS_TYPE_DE*0..3]->(root) RETURN count(n)` côté Cypher (inclut la racine), ou `WHERE id != root_id` côté SQL.
- Soit on l'accepte et on **compare des ordres de grandeur**, pas des nombres exacts. Ce qui compte ici, c'est : 1 ms vs 4 ms (SQL gagne sur cette taille), pas que ce soit "11 vs 10".

Annotez votre tableau benchmark pour préciser ce que chaque moteur a réellement compté.
:::
...
→ [Corrigé](../corriges/phase_3.qmd#t5--couper-1-arte)
...
→ [Corrigé](../corriges/phase_3.qmd#t6--couper-3-artes)
...
→ [Corrigé](../corriges/phase_3.qmd#t7--stratgie-dfensive)
...
→ [Corrigé](../corriges/phase_3.qmd#t8--stratgie-offensive)

---

## Partie C — Analyse de réseau (1h) — *Bonus / Aller plus loin*

> Ces tâches approfondissent l'analyse réseau. **Optionnelles** si le temps est limité — concentrez-vous sur la Partie B (simulation) et la Partie D (rapport).

### T9 — Composantes connexes : delta avant/après destruction

> Reprend la baseline de [T0](#sec-t0-etat-initial). Maintenant qu'on a coupé 5 arêtes (T8), on mesure le **delta**.

**Objectif** : comparer `pgr_connectedComponents` AVANT (T0) et APRÈS (T8) — quelles arêtes ont **vraiment** fragmenté le réseau ?

**Analysez** :

1. Composantes T0 (intact) → composantes T8 (5 arêtes coupées) : delta ?
2. Y a-t-il une coupe qui a créé une **nouvelle** composante (= un pont stratégique) ? Ou des coupes "à vide" qui n'ont rien isolé ?
3. Combien de **nouveaux** POIs sont devenus isolés ? Quel est leur rôle dominant (attaque/défense/...) ?

→ [Corrigé](../corriges/phase_3.qmd#t9--composantes-connexes)
...
→ [Corrigé](../corriges/phase_3.qmd#t10--centralit-de-degr)
...
- POIs des 3 rôles (couleurs distinctes)
...
## Partie D — T-Synthèse : Carte de Situation Commune (30 min)

> *Le livrable principal de la mission — les 3 perspectives fusionnées sur votre EPCI.*

### T-Synthèse — Fusionner les résultats du groupe

#### Étape 1 : chaque étudiant exporte ses tables finales
...
# Adaptez au nom de votre rôle
mon_role = "attaque"  # ou "defense" ou "logistique"

pd.read_sql(f"SELECT * FROM mission_pois WHERE role = '{mon_role}'", engine
            ).to_parquet(f"exports/mission_pois_{mon_role}.parquet", index=False)

# Tables spécifiques à chaque rôle (créées lors des tâches précédentes)
# Attaque :
pd.read_sql("SELECT * FROM attaque_targets", engine
            ).to_parquet("exports/attaque_targets.parquet", index=False)
# Défense :
# pd.read_sql("SELECT * FROM defense_choke_points", engine).to_parquet(...)
# Logistique :
# pd.read_sql("SELECT * FROM logistique_corridors", engine).to_parquet(...)
```

Déposez vos fichiers dans le dossier `exports/` du groupe.

#### Étape 2 : le « carteur » importe les 3 exports

Le carteur désigné importe les Parquets de ses coéquipiers dans sa BDD :
...
# Import croisé (si pas déjà fait en début de Phase 2)
for role in ["attaque", "defense", "logistique"]:
    df = pd.read_parquet(f"exports/mission_pois_{role}.parquet")
    df.to_sql("mission_pois", engine, if_exists="append", index=False,
              method="multi", chunksize=500)
```

#### Étape 3 : générer la Carte de Situation Commune

```python
from scripts.benchmark_comparison import generate_situation_map

generate_situation_map(
    epci="<votre EPCI>",  # ex. "Brest Métropole"
    layers=["attaque", "defense", "logistique"],
    output_path="exports/carte_situation_<epci>.png"
)
```

La carte superpose les POIs des 3 rôles (couleurs distinctes), les choke points, et les routes impactées par la simulation.

#### Étape 4 : mini-debrief écrit (5 lignes par rôle)

Chaque agent rédige 5 lignes dans le rapport commun :

- Ce que j'ai appris en analysant mon rôle sur cet EPCI
- Ce qui m'a surpris dans les données (un écart, une anomalie, une structure inattendue)
- Ce que je referais différemment avec plus de temps
- Un indicateur clé de mon rôle (nombre de POIs critiques, choke points détectés, etc.)
- En quoi la coopération avec les autres rôles a enrichi mon analyse

---

## Partie E — Rédaction du rapport (intégré à T-Synthèse)
