...
| `mission_custom_pois` | *(toutes)* | **Centrales nucléaires** (injectées au setup, **selon EPCI**) |

::: {.callout-warning title="⚛️ Centrales nucléaires : présence selon l'EPCI"}
La table `mission_custom_pois` n'est peuplée que si votre EPCI est **proche d'un site nucléaire** :

| EPCI | Centrale |
|------|----------|
| CU de Dunkerque | Gravelines |
| CA du Cotentin | Flamanville |
| CC de Cattenom et Environs | Cattenom |
| Ardenne Métropole | Chooz |
| CU du Grand Poitiers | Civaux |

Pour les **autres EPCIs** (Brest, Le Havre, Grenoble, Strasbourg, Lille, Pays Basque, Rouen, Bordeaux), `mission_custom_pois` est **vide**. Ce n'est pas un bug — la requête doit s'exécuter sans erreur et retourner `0 lignes`. Adaptez la communication de votre rapport en conséquence (« Pas de risque nucléaire direct dans l'EPCI »).
:::

## Requête de base

```sql
-- Postes de transformation critiques (importance 1-3 = sites majeurs)
-- Note : poste_de_transformation n'a PAS de colonne `nature` — on utilise `importance` comme catégorie
...
-- Centrales nucléaires (injectées par le setup, vide hors EPCIs nucléaires)
INSERT INTO mission_pois (role, source, cleabs, categorie, nature, nom, geom)
