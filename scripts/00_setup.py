...
import pandas as pd
from shapely import wkb, prepared
...
DEFAULT_SOURCE = "/media/stores/cifs/store-DAI/pocs/bd_search/postgis/data/parquet"
LOCAL_DATA = "data"
...
KEEP_COLS = {
    "batiment": ["fid", "cleabs", "nature", "usage_1", "usage_2", "etat_de_l_objet", "geometrie", "geometrie_bbox"],
    "troncon_de_route": ["fid", "cleabs", "nature", "nom_collaboratif_gauche", "nom_collaboratif_droite",
        "importance", "fictif", "position_par_rapport_au_sol", "nombre_de_voies", "largeur_de_chaussee",
        "sens_de_circulation", "vitesse_moyenne_vl", "restriction_de_hauteur", "restriction_de_poids_total",
        "restriction_de_poids_par_essieu", "restriction_de_largeur", "restriction_de_longueur",
        "matieres_dangereuses_interdites", "geometrie", "geometrie_bbox"],
    "construction_lineaire": ["fid", "cleabs", "nature", "nature_detaillee", "toponyme", "importance",
        "etat_de_l_objet", "geometrie", "geometrie_bbox"],
    "cours_d_eau": ["fid", "cleabs", "code_hydrographique", "toponyme", "statut",
        "influence_de_la_maree", "caractere_permanent", "geometrie", "geometrie_bbox"],
    "zone_d_activite_ou_d_interet": ["fid", "cleabs", "categorie", "nature", "nature_detaillee",
        "toponyme", "importance", "nom_commercial", "geometrie", "geometrie_bbox"],
    "zone_de_vegetation": ["fid", "cleabs", "nature", "nature_detaillee", "etat_de_l_objet",
        "geometrie", "geometrie_bbox"],
}
...
def list_epcis():
    df = pd.read_parquet(f"{LOCAL_DATA}/epci.parquet", columns=["code_siren", "nom_officiel"])
    print(f"\n{'SIREN':<15} | {'NOM EPCI'}")
    print("-" * 50)
...
def get_epci_info(query):
    ...
    mask = (df["code_siren"] == query) | (df["nom_officiel"].str.contains(query, case=False, na=False))
    res = df[mask]
    if res.empty:
        raise ValueError(f"EPCI '{query}' non trouvé.")
    if len(res) > 1:
        print(f"[WARNING] Plusieurs EPCI trouvés pour '{query}', on prend le premier : {res.iloc[0]['nom_officiel']}")
    
    row = res.iloc[0]
    geom = wkb.loads(row["geometrie"])
    bbox = row["geometrie_bbox"] # {'xmin':..., 'ymin':..., 'xmax':..., 'ymax':...}
    return row["nom_officiel"], row["code_siren"], geom, (bbox['xmin'], bbox['ymin'], bbox['xmax'], bbox['ymax'])
...
def filter_row_groups(pf, xmin, ymin, xmax, ymax):
    ...
    for rg_idx in range(pf.metadata.num_row_groups):
        ...
        if bb and bb["xmin"] < xmax and bb["xmax"] > xmin and bb["ymin"] < ymax and bb["ymax"] > ymin:
            kept.append(rg_idx)
    return kept
...
def create_table(conn, table_name, arrow_schema, cols_to_keep):
    with conn.cursor() as cur:
        ...
        for name, typ in zip(arrow_schema.names, arrow_schema.types):
            if name in ("geometrie", "geometrie_bbox") or (cols_to_keep and name not in cols_to_keep):
                continue
            pg_type = "TEXT"
            if "int64" in str(typ): pg_type = "BIGINT"
            elif "int" in str(typ): pg_type = "INTEGER"
            elif "float" in str(typ) or "double" in str(typ): pg_type = "DOUBLE PRECISION"
            elif "bool" in str(typ): pg_type = "BOOLEAN"
        ...
        if "geometrie" in arrow_schema.names:
            ...
            cur.execute(f"CREATE INDEX {table_name}_geom_idx ON {table_name} USING GIST (geometrie);")
        else:
            cur.execute(f"CREATE TABLE {table_name} ({', '.join(col_defs)});")
    conn.commit()
...
def insert_batch(conn, table_name, table_arrow, epci_geom_prepared, cols_to_keep):
    ...
    if has_geom:
        ...
        df = df[df["_shapely"].apply(lambda x: epci_geom_prepared.intersects(x) if x else False)]
    
    if df.empty:
        return 0

    cols = [c for c in df.columns if c not in ("geometrie", "geometrie_bbox", "_shapely") and (not cols_to_keep or c in cols_to_keep)]
    ...
    if has_geom:
        insert_sql = f"INSERT INTO {table_name} ({', '.join(quoted_cols)}, geometrie) VALUES %s"
    else:
...
def load_ontology(conn):
    ...
    with conn.cursor() as cur:
        for name in df_db["name"].unique():
            cur.execute("INSERT INTO bdtopo_ontology (name, obj_type) VALUES (%s, 'Database') RETURNING id;", (name,))
    ...
    with conn.cursor() as cur:
        for _, row in df_obj.iterrows():
            ...
            cur.execute("INSERT INTO bdtopo_ontology (name, obj_type, parent_id) VALUES (%s, 'Object', %s) RETURNING id;", 
                        (row["name"], parent_id))
    ...
    with conn.cursor() as cur:
        for _, row in df_det.iterrows():
            ...
            cur.execute("INSERT INTO bdtopo_ontology (name, obj_type, parent_id) VALUES (%s, 'Detail', %s);", 
                        (row["name"], parent_id))
...
def inject_custom_pois(conn, epci_geom_prepared):
    ...
    proj_l93 = "EPSG:2154"
    import pyproj
    transformer = pyproj.Transformer.from_crs(proj_wgs84, proj_l93, always_xy=True).transform
...
def main():
    ...
    parser.add_argument("--list-epci", action="store_true", help="Lister les EPCI disponibles")
    parser.add_argument("--source", default=DEFAULT_SOURCE, help="Dossier source des Parquets BDTOPO")
    ...
    for table_name in TARGET_TABLES:
        ...
        inserted = 0
        for rg_id in rg_ids:
            table_arrow = pf.read_row_group(rg_id, columns=cols_to_keep + ["geometrie"] if cols_to_keep and "geometrie" in pf.schema_arrow.names else (cols_to_keep if cols_to_keep else None))
            inserted += insert_batch(conn, table_name, table_arrow, prep_geom, cols_to_keep)
